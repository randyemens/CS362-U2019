...
int main () {
	...
	asserttrue(mineCase(&testState, 1, 0, 4) == -1, 1);
	...
	testState.hand[0][1] = mine;
	testState.whoseTurn = 0;
	asserttrue(mineCase(&testState, 1, 0, 3) == -1, 1);
	printf("Choice2 above 6 returns -1: ");
	initializeGame(4, k, 30, &testState);
	testState.handCount[0] = 2;
	testState.hand[0][0] = gold;
	testState.hand[0][1] = mine;
	asserttrue(mineCase(&testState, 1, 0, 7) == -1, 1);
	printf("Choosing card that is over +3 cost returns -1: ");
	initializeGame(4, k, 30, &testState);
	testState.handCount[0] = 2;
	testState.hand[0][0] = 4;
	testState.hand[0][1] = mine;
	asserttrue(mineCase(&testState, 1, 0, 6) == -1, 1);
	printf("\n------------------------------------------------------------------------------------------\n");
	...
	asserttrue(mineCase(&testState, 0, 1, silver) == 0, 1);
