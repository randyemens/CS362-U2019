...
int main () {
	...
	asserttrue(ambassadorCard(2, 3, &testState, 0, 2, 0) == -1, 1);
	printf("Choice 2 cannot be greater than 2: ");
	asserttrue(ambassadorCard(0, 3, &testState, 0, 2, 0) == -1, 1);
	printf("Choice 2 cannot be less than 0: ");
	asserttrue(ambassadorCard(0, -1, &testState, 0, 2, 0) == -1, 1);
	...
	testState.discardCount[0] = 0;
	asserttrue(ambassadorCard(0, 2, &testState, 0, 1, 0) == -1, 1);
	...
	testState.hand[0][2] = ambassador;
	testState.discardCount[0] = 0;
	asserttrue(ambassadorCard(0, 2, &testState, 0, 1, 0) == 0, 1);
	printf("\n------------------------------------------------------------------------------------------\n");
	...
	for (j = 1; j < 3; j++)
	{
		...
		asserttrue(ambassadorCard(3, j, &testState, 0, 0, 1) == 0, 1);
